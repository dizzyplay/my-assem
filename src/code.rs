...
pub fn jump(symbol: String) -> String {
    match symbol.as_str() {
        ...
        "JMP" => format!("111"),
        _ => {
            panic!("wrong symbol {}", symbol.as_str())
        }
    }
}
...
fn jump_test() {
    ...
    assert_eq!(jump(format!("JGT")), "001");
    assert_eq!(jump(format!("JEQ")), "010");
    assert_eq!(jump(format!("JGE")), "011");
    assert_eq!(jump(format!("JLT")), "100");
    assert_eq!(jump(format!("JNE")), "101");
    assert_eq!(jump(format!("JLE")), "110");
    assert_eq!(jump(format!("JMP")), "111");
}
